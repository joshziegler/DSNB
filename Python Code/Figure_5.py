...
M = np.linspace(0.1,120.,150) #Msun

lum = np.where(M<2., 
    M**4.8, 
    np.where(M<20., 
        2**(1.3)*M**3.5, 
        2**(201./50.)*5**(34./25.) * M**2.14)) #Lsun
        
rad = np.where(M<10., 
    M**0.8, 
    10**(9./20.)*M**0.35) #Rsun
    
temp = 5777*np.where(M<2., 
    M**0.8, 
    np.where(M<10., 
        2**(13./40.)*M**0.475, 
        np.where(M<20., 
...
for j in range(len(M)):
    for i in range(len(z)):
        zdi = zd(z[i])[j]
        totstarlum[j,i,:] = integ.simps(spec[j][:,(z<=z[i]) & (z>=zdi)],
            x= z[(z<=z[i]) & (z>=zdi)], axis=1)

logL = np.linspace(8,14,200)
L = 10**logL

sfrimf = integ.simps([(sn.RSF(L)*sn.IMF(M,L).T)[i,:] 
    * (sn.Phi_Spiral(L,z) + sn.Phi_Starburst(L,z) + sn.Phi_SFAGN(L,z)) 
    for i in range(len(M))], x=logL, axis=2)
    
sfrimfsal = integ.simps([(sn.RSF(L, usesalpeter=True)*sn.IMF(M,L,usesalpeter=True).T)[i,:] 
    * (sn.Phi_Spiral(L,z) + sn.Phi_Starburst(L,z) + sn.Phi_SFAGN(L,z)) 
    for i in range(len(M))], x=logL, axis=2)

integrand = integ.simps([sfrimf*totstarlum[:,:,i] for i in range(len(E))], x=M, axis=1)
integrandsal = integ.simps([sfrimfsal*totstarlum[:,:,i] for i in range(len(E))], x=M, axis=1)

ssrd = integ.simps(sn.Hinv(z)*integrand, x=z, axis=1)
ssrdsal = integ.simps(sn.Hinv(z)*integrandsal, x=z, axis=1)

ebl = E**2*sn.c/(4*np.pi) * 1.60217662e-12/3.086e22**2 * ssrd/(2*np.pi) #erg s^-1 m^-2 sr^-1
...
BW_lambda, BW_flux, BW_lambda_error_minus, BW_lambda_error_plus, BW_flux_error = np.loadtxt("../data/BW2015.txt", unpack=True)
plt.errorbar(
    BW_lambda, 
    BW_flux, 
